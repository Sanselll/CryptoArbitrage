...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Create real database if it doesn't exist
    SELECT 'CREATE DATABASE crypto_arbitrage_real'
    WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'crypto_arbitrage_real')\gexec

    -- Grant all privileges to the postgres user
    GRANT ALL PRIVILEGES ON DATABASE crypto_arbitrage_demo TO $POSTGRES_USER;
    GRANT ALL PRIVILEGES ON DATABASE crypto_arbitrage_real TO $POSTGRES_USER;
EOSQL

echo "Demo and Real databases created successfully!"
